...
static int ask_whether_verify_failed(Ghandles * g, const char *cond)
{
    ...
    snprintf(text, sizeof(text),
         ...
         g->vmname);
#endif
    snprintf(dontagain_param, sizeof(dontagain_param), "vanir-quid-%s:%s", g->vmname, cond);
...
static void mkghandles(Ghandles * g)
{
    ...
    g->inter_appviewer_lock_fd = open("/var/run/vanir/appviewer.lock",
            O_RDWR | O_CREAT, 0666);
...
static int run_clipboard_rpc(Ghandles * g, enum clipboard_op op) {
    ...
    switch (op) {
        case CLIPBOARD_COPY:
            ...
            service_call = "DEFAULT:QUBESRPC vanir.ClipboardCopy";
            break;
        case CLIPBOARD_PASTE:
            path_stdin = QUBES_CLIPBOARD_FILENAME;
            path_stdout = "/dev/null";
            service_call = "DEFAULT:QUBESRPC vanir.ClipboardPaste";
            break;
        default:
            /* not reachable */
            return 0;
    ...
    switch(pid=fork()) {
        ...
        case 0:
            execl(QREXEC_POLICY_PATH, "qrexec-policy", "--assume-yes-for-ask", "--just-evaluate",
                    "dummy_id", source_vm, g->vmname, "vanir.ClipboardPaste", "0", (char*)NULL);
            perror("execl");
            _exit(1);
        default:
            waitpid(pid, &status, 0);
...
static void exec_pacat(Ghandles * g)
{
    ...
    snprintf(logname, sizeof logname, "/var/log/vanir/pacat.%s.log",
         g->vmname);
    snprintf(old_logname, sizeof old_logname, "/var/log/vanir/pacat.%s.log.old",
         g->vmname);
...
static void get_protocol_version(Ghandles * g)
{
    ...
    if (version_major < QUBES_GUID_PROTOCOL_VERSION_MAJOR)
        ...
        snprintf(message, sizeof message, "%s %s \""
                ...
                "To update Dom0, use 'vanir-dom0-update' command or do it via vanir-manager\"",
                g->use_kdialog ? KDIALOG_PATH : ZENITY_PATH,
                g->use_kdialog ? "--sorry" : "--error --text ",
...
{
    fprintf(stream,
        "Usage: vanir-guid -d domain_id -N domain_name [options]\n");
    fprintf(stream, "\n");
...
{
    static char buf[256];
    snprintf(buf, sizeof(buf), "/var/run/vanir/guid-%s.%d",
         type, domid);
    return buf;
}
...
void restart_guid() {
    cleanup();
    execv("/usr/bin/vanir-guid", restart_argv);
    perror("execv");
}
...
int main(int argc, char **argv)
{
    ...
    if (!ghandles.nofork) {
        ...
        snprintf(dbg_log, sizeof(dbg_log),
                "/var/log/vanir/guid.%s.log", ghandles.vmname);
        snprintf(dbg_log_old, sizeof(dbg_log_old),
                "/var/log/vanir/guid.%s.log.old", ghandles.vmname);
    ...
    if (chdir("/var/run/vanir")) {
        perror("chdir /var/run/vanir");
        exit(1);
    }
    errno = 0;
    ...
    if ((unsigned)snprintf(cmd_tmp, sizeof(cmd_tmp), "/usr/bin/vanirdb-write -d %s "
             "/vanir-keyboard \"`/usr/bin/setxkbmap -print`\"",
