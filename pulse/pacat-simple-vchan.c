...
static void vchan_rec_callback(pa_mainloop_api *UNUSED(a),
    ...
    if (u->rec_stream && pa_stream_get_state(u->rec_stream) == PA_STREAM_READY) {
        ...
        while (libvchan_data_ready(u->rec_ctrl) >= (int)sizeof(cmd)) {
            ...
            switch (cmd) {
                case VANIR_PA_SOURCE_START_CMD:
...
int main(int argc, char *argv[])
{
    ...
    u.play_ctrl = libvchan_client_init(domid, VANIR_PA_SINK_VCHAN_PORT);
    if (!u.play_ctrl) {
        perror("libvchan_client_init");
        exit(1);
    }
    u.rec_ctrl = libvchan_client_init(domid, VANIR_PA_SOURCE_VCHAN_PORT);
    if (!u.rec_ctrl) {
        perror("libvchan_client_init");
        exit(1);
    }
